...
class AjaxController extends Controller
{
    ...
    public function get_division($state_id)
    {
//        $state_id = Qs::decodeHash($state_id);
//        return ['id' => Qs::hash($q->id), 'name' => $q->name];

        $lgas = $this->loc->getDivision($state_id);
        return $data = $lgas->map(function($q){
            return ['id' => $q->id, 'name' => $q->name];
        })->all();
    }

    public function get_upazila($state_id)
    {
//        $state_id = Qs::decodeHash($state_id);
//        return ['id' => Qs::hash($q->id), 'name' => $q->name];

        $lgas = $this->loc->getUpazila($state_id);
        return $data = $lgas->map(function($q){
            return ['id' => $q->id, 'name' => $q->upazila];
        })->all();
    }
